...
# Read data #
#############
dfTraits <- read_delim("./data/tblTraitSpecialisation.csv",
											 delim = ";") %>%
	filter(Trait == "Specialisation") %>%
	filter(nYears >= 2) %>%
...
rli_summary_Monophagous <- data.frame(
	year = c(2010, 2025),
...
head(rli_summary_Monophagous)

rli_summary_Monophagous$Specialisation <- "Monophagous"
head(rli_summary_Monophagous)

df_summary_Monophagous <- tibble(
	mean_difference = mean_diff,
	lower_ci = ci_diff[1],
	upper_ci = ci_diff[2],
	n = nrow(df_wide)
) %>%
	mutate(Specialisation = "Monophagous") %>%
	select(Specialisation, n, lower_ci, mean_difference, upper_ci)
df_summary_Monophagous

################
# Oligophagous #
################
df <- dfTraitRLC %>%
	filter(TraitValue == "Oligophagous") %>%
	filter(TraitValue != "Range extends outside Palearctic and Holarctic") %>%
	filter(!is.na(RLC))
head(df)
nrow(df)
...
rli_summary_Oligophagous <- data.frame(
	year = c(2010, 2025),
	rli_mean = c(mean(rli_2010), mean(rli_2025)),
	rli_lower = c(quantile(rli_2010, 0.025), quantile(rli_2025, 0.025)),
	rli_upper = c(quantile(rli_2010, 0.975), quantile(rli_2025, 0.975))
)
head(rli_summary_Oligophagous)

rli_summary_Oligophagous$Specialisation <- "Oligophagous"
head(rli_summary_Oligophagous)

df_summary_Oligophagous <- tibble(
	mean_difference = mean_diff,
	lower_ci = ci_diff[1],
	upper_ci = ci_diff[2],
	n = nrow(df_wide)
) %>%
	mutate(Specialisation = "Oligophagous") %>%
	select(Specialisation, n, lower_ci, mean_difference, upper_ci)
df_summary_Oligophagous

###############
# Polyphagous #
###############
df <- dfTraitRLC %>%
	filter(TraitValue == "Polyphagous") %>%
	filter(TraitValue != "Range extends outside Palearctic and Holarctic") %>%
	filter(!is.na(RLC))
head(df)
nrow(df)
...
rli_summary_Polyphagous <- data.frame(
	year = c(2010, 2025),
	rli_mean = c(mean(rli_2010), mean(rli_2025)),
	rli_lower = c(quantile(rli_2010, 0.025), quantile(rli_2025, 0.025)),
	rli_upper = c(quantile(rli_2010, 0.975), quantile(rli_2025, 0.975))
)
head(rli_summary_Polyphagous)

rli_summary_Polyphagous$Specialisation <- "Polyphagous"
head(rli_summary_Polyphagous)

df_summary_Polyphagous <- tibble(
	mean_difference = mean_diff,
	lower_ci = ci_diff[1],
	upper_ci = ci_diff[2],
	n = nrow(df_wide)
) %>%
	mutate(Specialisation = "Polyphagous") %>%
	select(Specialisation, n, lower_ci, mean_difference, upper_ci)
df_summary_Polyphagous

##########################
# Put all RLI's together #
##########################
mean_differences <- rbind(df_summary_Monophagous,
													df_summary_Oligophagous,
													df_summary_Polyphagous) %>%
...
rli_summary_all <- rbind(rli_summary_Monophagous,
												 rli_summary_Oligophagous,
												 rli_summary_Polyphagous)
...
final_labels <- rli_summary_all %>%
	...
	left_join(
		mean_differences %>%
			select(Specialisation, effect_code),
		by = c("Specialisation" = "Specialisation")
	)
final_labels

rli_summary_all$Specialisation <- factor(rli_summary_all$Specialisation,
																				 levels = c("Monophagous",
																				 					 "Oligophagous",
																				 					 "Polyphagous"))
# Without 1999
...
p <- ggplot(rli_summary_all,
	...
	ggrepel::geom_label_repel(
		data = final_labels,
		aes(label = as.character(effect_code), fill = Specialisation),
		box.padding = 0,
	...
	labs(color = "Specialisation") +
	ggtitle("Specialisation")
p

dir.create("output/figures/rli", recursive = TRUE, showWarnings = FALSE)
ggsave("./output/figures/rli/RLI_EuropeanButterflies2010_2025_HostPlantSpecialisation.jpg",
			 width = 6,
			 height = 4,
			 dpi = 150)
...
###############################
df_summary_specialisation <- rbind(df_summary_Monophagous,
																	 df_summary_Oligophagous,
																	 df_summary_Polyphagous) %>%
	mutate(
		n_specs = mean_difference * n * 5,
		label = paste0(Specialisation, " (n = ", n, ")")
	) %>%
	dubicube::add_effect_classification(
...
df_summary_specialisation

df_summary_specialisation$Specialisation <- factor(df_summary_specialisation$Specialisation,
																									 levels = c("Polyphagous",
																									 					 "Oligophagous",
																									 					 "Monophagous"))
...
ggsave("./output/figures/diff_rli/diffRLI_EuropeanButterflies2010_2025_HostPlantSpecialisation.jpg",
			 width = 12,
			 height = 5,
			 dpi = 150)
