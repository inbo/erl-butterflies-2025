...
# Read data #
#############
dfTraits <- read_delim("./data/tblTraitRangeSize.csv",
											 delim = ";") %>%
	filter(Trait == "RangeSize") %>%
	filter(nYears >= 2) %>%
...
rli_summary_VerySmall <- data.frame(
	year = c(2010, 2025),
...
head(rli_summary_VerySmall)

rli_summary_VerySmall$RangeSize <- "VerySmall"
head(rli_summary_VerySmall)

df_summary_VerySmall <- tibble(
	mean_difference = mean_diff,
	lower_ci = ci_diff[1],
	upper_ci = ci_diff[2],
	n = nrow(df_wide)
) %>%
	mutate(RangeSize = "Very small") %>%
	select(RangeSize, n, lower_ci, mean_difference, upper_ci)
df_summary_VerySmall

#########
# Small #
#########
df <- dfTraitRLC %>%
	filter(TraitValue == "Small") %>%
	filter(TraitValue != "Range extends outside Palearctic and Holarctic") %>%
	filter(!is.na(RLC))
head(df)
nrow(df)
...
rli_summary_Small <- data.frame(
	year = c(2010, 2025),
	rli_mean = c(mean(rli_2010), mean(rli_2025)),
	rli_lower = c(quantile(rli_2010, 0.025), quantile(rli_2025, 0.025)),
	rli_upper = c(quantile(rli_2010, 0.975), quantile(rli_2025, 0.975))
)
head(rli_summary_Small)

rli_summary_Small$RangeSize <- "Small"
head(rli_summary_Small)

df_summary_Small <- tibble(
	mean_difference = mean_diff,
	lower_ci = ci_diff[1],
	upper_ci = ci_diff[2],
	n = nrow(df_wide)
) %>%
	mutate(RangeSize = "Small") %>%
	select(RangeSize, n, lower_ci, mean_difference, upper_ci)
df_summary_Small
...
rli_summary_Large <- data.frame(
	year = c(2010, 2025),
	rli_mean = c(mean(rli_2010), mean(rli_2025)),
	rli_lower = c(quantile(rli_2010, 0.025), quantile(rli_2025, 0.025)),
	rli_upper = c(quantile(rli_2010, 0.975), quantile(rli_2025, 0.975))
)
head(rli_summary_Large)

rli_summary_Large$RangeSize <- "Large"
head(rli_summary_Large)

df_summary_Large <- tibble(
	mean_difference = mean_diff,
	lower_ci = ci_diff[1],
	upper_ci = ci_diff[2],
	n = nrow(df_wide)
) %>%
	mutate(RangeSize = "Large") %>%
	select(RangeSize, n, lower_ci, mean_difference, upper_ci)
df_summary_Large
...
rli_summary_VeryLarge <- data.frame(
	year = c(2010, 2025),
	rli_mean = c(mean(rli_2010), mean(rli_2025)),
	rli_lower = c(quantile(rli_2010, 0.025), quantile(rli_2025, 0.025)),
	rli_upper = c(quantile(rli_2010, 0.975), quantile(rli_2025, 0.975))
)
head(rli_summary_VeryLarge)

rli_summary_VeryLarge$RangeSize <- "VeryLarge"
head(rli_summary_VeryLarge)

df_summary_VeryLarge <- tibble(
	mean_difference = mean_diff,
	lower_ci = ci_diff[1],
	upper_ci = ci_diff[2],
	n = nrow(df_wide)
) %>%
	mutate(RangeSize = "Very large") %>%
	select(RangeSize, n, lower_ci, mean_difference, upper_ci)
df_summary_VeryLarge

##########################
# Put all RLI's together #
##########################
mean_differences <- rbind(df_summary_VerySmall,
													df_summary_Small,
													df_summary_Large,
													df_summary_VeryLarge) %>%
...
rli_summary_all <- rbind(rli_summary_VerySmall,
												 rli_summary_Small,
												 rli_summary_Large,
												 rli_summary_VeryLarge)
rli_summary_all
...
final_labels <- rli_summary_all %>%
	...
	left_join(
		mean_differences %>%
			select(RangeSize, effect_code),
		by = c("RangeSize" = "RangeSize")
	)

rli_summary_all$RangeSize <- factor(rli_summary_all$RangeSize,
																		levels = c("ExtremelySmall",
																							 "VerySmall",
																							 "Small",
																							 "Large",
																							 "VeryLarge"))
...
p <- ggplot(rli_summary_all,
	...
	ggrepel::geom_label_repel(
		data = final_labels,
		aes(label = as.character(effect_code), fill = RangeSize),
		box.padding = 0,
...
ggsave("./output/figures/rli/RLI_EuropeanButterflies2010_2025_RangeSize.jpg",
			 width = 6,
			 height = 4,
			 dpi = 150)
...
###############################
df_summary_rangesize <- rbind(df_summary_VerySmall,
															df_summary_Small,
															df_summary_Large,
															df_summary_VeryLarge) %>%
	mutate(
		n_specs = mean_difference * n * 5,
		label = paste0(RangeSize, " (n = ", n, ")")
	) %>%
	dubicube::add_effect_classification(
...
df_summary_rangesize

df_summary_rangesize$RangeSize <- factor(df_summary_rangesize$RangeSize,
																		levels = c("ExtremelySmall",
																							 "VerySmall",
																							 "Small",
																							 "Large",
...
ggsave("./output/figures/diff_rli/diffRLI_EuropeanButterflies2010_2025_rangeSize.jpg",
			 width = 12,
			 height = 5,
			 dpi = 150)
