...
# Read data #
#############
dfTraits <- read_delim("./data/tblTraitWingspan.csv",
											 delim = ";") %>%
	filter(Trait == "Wingspan") %>%
	filter(nYears >= 2) %>%
...
rli_summary_VerySmall$Wingspan <- "VerySmall"
head(rli_summary_VerySmall)
...
df_summary_VerySmall <- tibble(
	...
	mutate(Wingspan = "Very small") %>%
	select(Wingspan, n, lower_ci, mean_difference, upper_ci)
...
rli_summary_Small$Wingspan <- "Small"
head(rli_summary_Small)
...
rli_summary_Large$Wingspan <- "Large"
head(rli_summary_Large)
...
rli_summary_VeryLarge$Wingspan <- "VeryLarge"
head(rli_summary_VeryLarge)
...
final_labels <- rli_summary_all %>%
	...
	left_join(
		mean_differences %>%
			select(Wingspan, effect_code),
		by = c("Wingspan" = "Wingspan")
	)
final_labels

rli_summary_all$Wingspan <- factor(rli_summary_all$Wingspan,
																		levels = c("VerySmall",
																							 "Small",
																							 "Large",
																							 "VeryLarge"))
...
p <- ggplot(rli_summary_all,
	...
	ggrepel::geom_label_repel(
		data = final_labels,
		aes(label = as.character(effect_code), fill = Wingspan),
		box.padding = 0,
...
ggsave("./output/figures/rli/RLI_EuropeanButterflies2010_2025_Wingspan.jpg",
			 width = 6,
			 height = 4,
			 dpi = 150)
...
###############################
df_summary_wingspan <- rbind(df_summary_VerySmall,
															df_summary_Small,
															df_summary_Large,
															df_summary_VeryLarge) %>%
	mutate(
		n_specs = mean_difference * n * 5,
		label = paste0(Wingspan, " (n = ", n, ")")
	) %>%
	dubicube::add_effect_classification(
...
df_summary_wingspan$Wingspan <- factor(df_summary_wingspan$Wingspan,
																				 levels = c("Very large",
																				 					 "Large",
																				 					 "Small",
																				 					 "Very small"))
...
ggsave("./output/figures/diff_rli/diffRLI_EuropeanButterflies2010_2025_wingspan.jpg",
			 width = 12,
			 height = 5,
			 dpi = 150)
