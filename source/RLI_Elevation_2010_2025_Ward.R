...
head(rli_summary_Low)

rli_summary_Low$Elevation <- "Lowland"
head(rli_summary_Low)

df_summary_Low <- tibble(
...
rli_summary_Intermediate <- data.frame(
	year = c(2010, 2025),
	rli_mean = c(mean(rli_2010), mean(rli_2025)),
	rli_lower = c(quantile(rli_2010, 0.025), quantile(rli_2025, 0.025)),
	rli_upper = c(quantile(rli_2010, 0.975), quantile(rli_2025, 0.975))
)
head(rli_summary_Intermediate)

rli_summary_Intermediate$Elevation <- "Intermediate"
head(rli_summary_Intermediate)

df_summary_Intermediate <- tibble(
	mean_difference = mean_diff,
	lower_ci = ci_diff[1],
	upper_ci = ci_diff[2],
	n = nrow(df_wide)
...
rli_summary_High <- data.frame(
	year = c(2010, 2025),
	rli_mean = c(mean(rli_2010), mean(rli_2025)),
	rli_lower = c(quantile(rli_2010, 0.025), quantile(rli_2025, 0.025)),
	rli_upper = c(quantile(rli_2010, 0.975), quantile(rli_2025, 0.975))
)
head(rli_summary_High)

rli_summary_High$Elevation <- "Upland"
head(rli_summary_High)

df_summary_High <- tibble(
	mean_difference = mean_diff,
	lower_ci = ci_diff[1],
	upper_ci = ci_diff[2],
	n = nrow(df_wide)
...
rli_summary_all <- rbind(#rli_summary_VeryLow,
												 rli_summary_Low,
												 rli_summary_Intermediate,
												 rli_summary_High)
												 #rli_summary_VeryHigh)
rli_summary_all
...
final_labels <- rli_summary_all %>%
	...
	left_join(
		mean_differences %>%
			select(Elevation, effect_code),
		by = c("Elevation" = "Elevation")
	)
final_labels

rli_summary_all$Elevation <- factor(rli_summary_all$Elevation,
																	 levels = c(#"VeryLow",
																	 					 "Lowland",
																	 					 "Intermediate",
																	 					 "Upland"))
...
p <- ggplot(rli_summary_all,
	...
	ggrepel::geom_label_repel(
		data = final_labels,
		aes(label = as.character(effect_code), fill = Elevation),
		box.padding = 0,
...
###############################
df_summary_elevation <- rbind(df_summary_Low,
															#df_summary_VeryLow,
															#df_summary_VeryHigh)
															df_summary_Intermediate,
															df_summary_High) %>%
...
df_summary_elevation

df_summary_elevation$Elevation <- factor(df_summary_elevation$Elevation,
																				 levels = c(#"VeryHigh",
																				 	"Upland",
																				 	"Intermediate",
																				 	"Lowland"))#,
